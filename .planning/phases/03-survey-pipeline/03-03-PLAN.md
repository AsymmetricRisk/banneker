...
files_modified:
  - lib/constants.js
  - lib/installer.js
  - test/integration/template-validation.test.js
autonomous: true
...
must_haves:
  ...
  artifacts:
    ...
    - path: "lib/installer.js"
      ...
      contains: "agents"
    - path: "test/integration/template-validation.test.js"
...
<task type="auto">
  <name>Task 1: Update constants and installer for agents directory</name>
  <files>lib/constants.js, lib/installer.js</files>
  <action>
...
5. Also update the `uninstall` function reference -- the uninstaller (in `lib/uninstaller.js`) uses BANNEKER_FILES. Check if it handles subdirectory paths like `agents/banneker-surveyor.md`. If the uninstaller resolves files relative to `commandsDir`, the agents path won't resolve correctly. Check and fix:
   - Files without a `/` are in commandsDir
   - Files starting with `agents/` are in configDir + path

   If the uninstaller needs updating, update it. If it already handles paths correctly, leave it.
  </action>
  <verify>
```bash
...
<task type="auto">
  ...
  <files>test/integration/template-validation.test.js</files>
  <action>
...
<task type="auto">
  ...
  <files>test/smoke/install-smoke.test.js</files>
  <action>
