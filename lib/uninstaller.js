...
export async function uninstall(commandsDir, configDir) {
  ...
  try {
    ...
    for (const filename of BANNEKER_FILES) {
      ...
      // Skip agent files if configDir is not provided (backwards compatibility)
      if (filename.startsWith('agents/')) {
        if (!configDir) {
          continue; // Skip agent files if no configDir provided
        }
    ...
    // Try to remove empty agents directory (only if configDir provided)
    if (configDir) {
